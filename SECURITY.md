# Update Checker â€” security model
...
the checker sits between three parties: the **local user config** (trusted),
...
### 1. Remote text â†’ prompt injection (LLM consumers)
...
**Posture: the checker fetches exactly one remote field â€” the release
...
only, and (b) be treated as untrusted data by any LLM consumer â€” labeled as
such wherever it lands in logs or UI. Update this file when that surface is
added.

### 2. Remote text â†’ command injection
...
passing a tag argument at all. `execFile` is used exclusively â€” no shell,
no interpolation, `windowsHide: true`.

### 3. Remote text â†’ DOM injection
...
config â€” unexpected assets in a release are simply not downloaded. GitHub
...
What the checker installs is executable plugin code: a compromised repo or
...
PATH. A PATH-hijacked `gh` implies the machine is already compromised â€”
out of scope.

### 7. Self-update

the checker downloads its own updates but refuses to hot-reload itself; the
user reloads it manually. No code path exec()s downloaded content â€” Obsidian
loads it on plugin reload like any plugin.

## Edge cases verified

- Repo with no releases â†’ "no releases", no crash.
- gh unauthenticated â†’ labeled error, no retry loop.
- Non-semver tag â†’ parses to zeros, worst case a wrong compare, never code.
- Asset missing from a release â†’ gh error surfaced as clipped text.
