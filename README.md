...
fleet of vaults â€” the niche BRAT covers, minus token management: auth is
...
- **Update** runs `gh release download` for the configured asset names into
  the plugin folder, then offers an explicit **Reload plugin** button â€”
...
See [SECURITY.md](SECURITY.md) â€” the checker fetches exactly one remote field
